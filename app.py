...
def load_user_data(user_id):
    ...
    manual_dividends = {row[0]: row[1] for row in cursor.fetchall()}
    ...
    for member, month, amount in div_rows:
...
def save_user_data(user_id):
    ...
    manual_dividends = req_data.get('manual_dividends', {})
    custom_dividends_list = req_data.get('custom_dividends_list', {})
    ...
    for stock_name, div_val in manual_dividends.items():
        cursor.execute('''
    ...
    for member, items in custom_dividends_list.items():
        for item in items:
            cursor.execute('''
